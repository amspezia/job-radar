...
async def search(
    session: AsyncSession, query: str, *, limit: int = 20, pool: int = 50
) -> list[Job]:
    query_embedding = await embed(query)
    fts_results = await search_fts(session, query, pool)
    vector_results = await search_vector(session, query_embedding, pool)
